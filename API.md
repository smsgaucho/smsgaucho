# SMSGAUCHO WEBSERVICE

This document explains about webservices protocol.

Minimum webservice version **1.0-rc5**


## Access

Webservices URL:
`http://www.smsgaucho.com.br/painel/index.php?app=ws`
...
example: `http://www.smsgaucho.com.br/painel/index.php?app=ws&u=<admin user>&h=<webkey>&op=stoplist&from=<number to be added>`
...
```
http://www.smsgaucho.com.br/painel/index.php?app=ws&u=anton&h=a45a02791b2fe2fedb078c39fd83637a&op=pv&to=0987654321&msg=test+only
```

Explanation:

The webservices in SMSGAUCHO with  peration `op` pv (send SMS) was accessed by a user using username `u` and webservices token `h` with destination number `to` 0987654321, message `msg` 'test only' and expected output format is the default format, JSON format.

When succeeded webservice will returns OK status message in JSON format:

```
{"data":[{"status":"OK","error":"0","smslog_id":"30","queue":"afb5f34575e30ec4efe4471cf5d1bee4","to":"0987654321"}],"error_string":null}
```
When error occurred webservices will returns one of the return code, also in JSON format.
...
```
http://www.smsgaucho.com.br/painel/index.php?app=ws&u=anton&h=482ac0069592c647289e52dfef88be68&op=in&kwd=IDOL&format=xml
```

Explanation:

Webservices in SMSGAUCHO with operation `op` in (incoming SMS) was accessed by a user using username `u` and webservices token `h` with keyword `kwd` IDOL and expected output format is in XML format `format=xml`.

When succeeded webservices will returns OK status message in XML format:
...
When error occurred webservices will returns one of the return code, also in XML format.
...
```
http://www.smsgaucho.com.br/painel/index.php?app=ws&u=anton&h=482ac0069592c647289e52dfef88be68&op=get_contact&kwd=anton
```

Explanation:

Webservices in SMSGAUCHO with operation `op` get_contact was accessed by a user using username `u` and webservices token `h` with keyword `kwd` anton and expected output format is in JSON format.

When succeeded webservices will returns OK status message in JSON format:
